...
- **⑤ 学习沉淀**（neutral 类别，N=3 时：前 2 次人工确认，之后进入阈值状态）
  - 阈值前：一律人工确认，**批准** → 计数 +1 并记录本次**操作指纹**（路径/文件名/项目名）；**拒绝** → 升级 denyRules
  - 阈值后（计数 ≥ N-1）：**只自动放行「指纹在确认历史中」的操作**，并沉淀带指纹的规则 `{tool, mode, category, contains}`（以后该指纹对应的操作直接放行，不过 flash）
  - **未确认过的操作**（不同指纹/混合确认）→ 即使计数达标也**继续人工确认**，绝不因同类操作被确认过而放行
  - 用户**拒绝** → 升级进 denyRules（带指纹；提取不到指纹则拦全部同类，拒绝从严）
  - 取消/不可用 → 不计数（用户未表态，下次仍人工确认）
  - 指纹提取不到（justification 无路径/文件名/项目名特征）→ 不沉淀，保持人工确认
...
3. **学习规则带类别 + 操作指纹**：沉淀的是 `{tool, mode, category, contains}`（contains = 用户确认过的操作指纹），只放行同一指纹的操作；确认过的操作 A 不会放行未确认的操作 B；拒绝过的操作升级 denyRules（带指纹，提取不到则拦全部同类），永不自动放行
